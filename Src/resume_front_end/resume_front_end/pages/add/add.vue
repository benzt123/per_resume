...
<script>
import { addExperienceRecord, updateExperienceRecord } from '@/utils/globalMethods.js'
		...
		methods: {
			...
			async createRecord() {
				const record = {
					...
					fileContent: '文件已上传，等待进一步处理'
				}

				try {
					const saved = await addExperienceRecord(record)
					uni.showToast({ title: '经历已保存', icon: 'success' })
					setTimeout(() => uni.navigateBack(), 1200)
					return saved
				} catch (e) {
					console.error('保存记录失败', e)
					uni.showToast({ title: '保存失败，已保存在本地', icon: 'none' })
					// fallback handled in addExperienceRecord
					setTimeout(() => uni.navigateBack(), 1200)
				}
			},
			...
			async updateRecord() {
				const patch = {
					...
					endDate: this.endDate,
					note: this.note.trim(),
					category: this.aiClassification ? this.aiClassification.category : undefined,
					summary: this.aiClassification ? this.aiClassification.summary : undefined
				}
				try {
					await updateExperienceRecord(this.editingRecordId, patch)
					uni.showToast({ title: '经历已更新', icon: 'success' })
					setTimeout(() => uni.navigateBack(), 1200)
				} catch (e) {
					console.error('更新记录失败', e)
					uni.showToast({ title: '更新失败，已在本地保存', icon: 'none' })
					setTimeout(() => uni.navigateBack(), 1200)
				}
			},
