...
<script>
import { getExperienceRecords, updateExperienceRecord } from '@/utils/globalMethods.js'
		...
		methods: {
			...
			async loadData() {
				...
				try {
					...
					this.allRecords = (records || []).slice().sort((a, b) => new Date(b.createdTime) - new Date(a.createdTime))
				} catch (e) {
					console.warn('loadData: 从后端获取 records 失败，回退本地', e)
			...
			async confirmCategoryChange() {
				...
				try {
					await updateExperienceRecord(this.editingRecord.id, { category: this.selectedCategory })
					uni.showToast({ title: '分类修改成功', icon: 'success' })
					this.loadData()
				} catch (e) {
					console.error('confirmCategoryChange failed', e)
					uni.showToast({ title: '分类修改失败', icon: 'none' })
				}
